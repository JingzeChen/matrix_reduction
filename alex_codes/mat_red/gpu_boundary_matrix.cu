...
__global__ void transform_all_columns(indx **tmp_gpu_columns, size_t *column_length, column *matrix, int column_num,
                                      ScatterAllocator::AllocatorHandle allocator, dimension *dims) {
    int thread_id = threadIdx.x + blockDim.x * blockIdx.x;
    if (thread_id >= column_num)
        return;
...
gpu_boundary_matrix::gpu_boundary_matrix(phat::boundary_matrix<phat::vector_vector> *src_matrix,
    ...
    gpuErrchk(cudaMemcpy(leftmost_lookup_lowest_row, h_leftmost_lookup_lowest_row, sizeof(indx) * cols_num,
    ...
    //gpuErrchk(cudaMalloc((void **) &leftmost_lookup_lowest_row, sizeof(indx *) * cols_num));
    allocate_all_columns << < CUDA_BLOCKS_NUM(cols_num), CUDA_THREADS_EACH_BLOCK(cols_num) >> >
                                                         (tmp_gpu_columns, d_column_length, cols_num, allocator);
...
__device__ void check_lowest_one_locally(column *matrix, unsigned long long *chunk_columns_finished, dimension *dims,
                                         bool *is_reduced, indx *leftmost_lookup_lowest_row, int thread_id,
                                         int block_id, indx column_start, indx column_end, indx row_begin, indx row_end,
                                         dimension cur_dim, indx num_cols, indx *target_col, bool *ive_added) {

    indx my_lowest_one = get_max_index(matrix, thread_id);
    if (cur_dim != get_dim(dims, thread_id) || is_reduced[thread_id] || thread_id >= num_cols ||
        my_lowest_one == -1) {
        if (!*ive_added) {
    ...
    if (my_lowest_one >= row_begin && my_lowest_one < row_end) {
        ...
        if (leftmost_lookup_lowest_row[my_lowest_one] >= column_start &&
            leftmost_lookup_lowest_row[my_lowest_one] < thread_id) {
...
__device__ void add_two_columns(column *matrix, indx target, indx source, ScatterAllocator::AllocatorHandle allocator) {
    ...
    auto new_value = (need_new_mem) ? (unsigned long long *) allocator.malloc(sizeof(unsigned long long) * max_size)
                                    : new unsigned long long[max_size];
    ...
    if (src_id < matrix[source].data_length) {
        ...
        memcpy(&new_value[temp_id], &matrix[source].value[src_id],
               sizeof(unsigned long long) * (matrix[source].data_length - src_id));
    ...
    if (tgt_id < matrix[target].data_length) {
        ...
        memcpy(&new_value[temp_id], &matrix[target].value[tgt_id],
               sizeof(unsigned long long) * (matrix[target].data_length - tgt_id));
...
__global__ void update_table(column *matrix, indx *leftmost_lookup_lowest_row, indx cur_phase, indx block_size) {
    ...
    indx column_start = ((blockDim.x * blockIdx.x - cur_phase * block_size) > 0) ? (blockDim.x * blockIdx.x -
                                                                                    cur_phase * block_size) : 0;
    ...
    __syncthreads();
    if (thread_id >= column_start &&
        thread_id < column_end)// && this_max_index >= row_begin && this_max_index < row_end)
    {
