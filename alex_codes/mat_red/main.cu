...
__global__ void gpu_spectral_sequence_reduction(column *matrix, unsigned long long *chunk_columns_finished,
        dimension *dims, indx *leftmost_lookup_lowest_row, dimension cur_dim, indx block_size, indx num_cols,
        indx *lowest_one_lookup, bool *is_reduced, indx cur_phase, ScatterAllocator::AllocatorHandle allocator) {

    int thread_id = threadIdx.x + blockDim.x * blockIdx.x;
    int block_id = blockIdx.x;
    int thread_in_id = threadIdx.x;
    ...
    indx row_begin = ((blockDim.x * blockIdx.x - cur_phase * block_size) > 0) ? (blockDim.x * blockIdx.x -
                                                                                 cur_phase * block_size) : 0;
...
int main(int argc, char **argv) {
        ...
        for (indx cur_phase = 0; cur_phase < block_num; cur_phase++) {
            gpu_spectral_sequence_reduction << < block_num, threads_block >> >
                                                            (g_matrix.matrix, g_matrix.chunk_columns_finished, g_matrix.dims,
                                                                    g_matrix.leftmost_lookup_lowest_row, cur_dim, threads_block,
                                                                    num_cols, g_matrix.lowest_one_lookup, g_matrix.is_reduced, cur_phase, allocator);
            cudaDeviceSynchronize();
    ...
    gpuErrchk(
            cudaMemcpy(&lookup_table[0], g_matrix.lowest_one_lookup, sizeof(indx) * num_cols, cudaMemcpyDeviceToHost));
    cudaDeviceSynchronize();
