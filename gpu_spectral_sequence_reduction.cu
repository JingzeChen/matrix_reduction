...
__global__ void gpu_spectral_sequence_reduction(column* matrix, dimension* dims, dimension cur_dim, indx block_size, indx num_cols, indx* lowest_one_lookup, bool* is_reduced, ScatterAllocator::AllocatorHandle allocator)
{
    int thread_id = threadIdx.x + blockDim.x * blockIdx.x;
    ...
    for(indx cur_phase = 0; cur_phase < phases ; cur_phase++)
    {
        ...
        while(!is_reduced[thread_id] && cur_lowest_one != -1 && cur_lowest_one >= row_begin && cur_lowest_one < row_end && lowest_one_lookup[cur_lowest_one] != -1)
        {
            add_two_columns(matrix, thread_id, lowest_one_lookup[cur_lowest_one], allocator);
            cur_lowest_one = get_max_index(matrix, thread_id);
        }
        if(cur_lowest_one != -1 && !is_reduced[thread_id])
        {
...
int main()
{
    ...
    for(dimension cur_dim = max_dim; cur_dim>=1; cur_dim--)
    {
        gpu_spectral_sequence_reduction<<<block_num, threads_block>>>(g_matrix.matrix, g_matrix.dims, cur_dim, block_size, num_cols, g_matrix.lowest_one_lookup, g_matrix.is_reduced, allocator);
    }
