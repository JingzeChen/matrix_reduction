...
namespace phat {
    ...
    public:
        ...
        void save_scs(
			...
			uint	cul_iNumSimplex [6 + 1]; // tants
			cul_iNumSimplex[0] = cul_iNumSimplex[1] = 0; // tants
            ...
            for( dimension_type dim = 0; dim < max_dim + 1; dim++ )
			{
				...
				if (dim >= 2) cul_iNumSimplex[dim] = cul_iNumSimplex[dim-1] + iNumSimplex[dim-2];  // tants
			}
            ...
            for( dimension_type dim = 0; dim < max_dim + 1; dim++ )
			{
				...
				for( index cur_col = 0; cur_col <  nr_of_columns; cur_col++ ) 
    	            if( get_dim( cur_col ) == dim )
					{
                		...
                		prefix += num_rows;
                		for( index cur_row = 0; cur_row <  num_rows; cur_row++ )
                    		iRow[dim][global_index++] = new_index[temp_col[cur_row]] + cul_iNumSimplex[dim]; // tants
					}
			}
		}
