...
namespace phat {
    class persistence_pairs {
    	typedef std::pair< index, index > persistence_pair;
    ...
    public:
        ...
        bool operator==( persistence_pairs& other_pairs ) {
			...
			{
				std::cerr<<"Different sizes: "<< pairs.size() << " ? "<< other_pairs.get_num_pairs() << "\n";
                //return false;
                res = false;
			...
			{
				persistence_pair a = get_pair(idx);
				persistence_pair b = other_pairs.get_pair(idx);
                if( get_pair( idx ) != other_pairs.get_pair( idx ) )
				{
					std::cerr << "Pair ("<<a.first<<","<<a.second<<") ? ("<<b.first<<","<<b.second<<")\n";  
                    //return false;
                    res = false;
				}
